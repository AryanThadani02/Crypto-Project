...
window.addEventListener("load", async()=>{
  const exchangeRate = await getDataFromApi("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=inr");
...
function showTrendingCoins(data,exchangeRate){
  data.forEach((coin)=>{
    ...
    price.innerText="₹ "+getCoinPriceInInr(coin.item.price_btc, exchangeRate);
